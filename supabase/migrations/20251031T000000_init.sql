-- Initial schema migration generated from repository database/schema.sql
...
CREATE POLICY IF NOT EXISTS "Service role full access users" ON users FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access profiles" ON garage_profiles FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access codes" ON verification_codes FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access registration_sessions" ON registration_sessions FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access files" ON file_uploads FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access bookings" ON bookings FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access appointments" ON appointments FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access inspections" ON inspections FOR ALL USING (auth.role() = 'service_role');
