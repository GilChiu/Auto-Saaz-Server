-- ============================================================
-- PRODUCTION-READY DEMO DATA SCRIPT
-- Based on confirmed working table structure
...
-- ============================================================
-- 1. CREATE DEMO USERS WITH COMPLETE BUSINESS PROFILES
-- ============================================================

-- Clear existing demo data (optional - uncomment to reset)
...
-- Insert 5 demo garage businesses
INSERT INTO users (
    id,
    email,
    role,
...
-- Business 1: Premium Auto Care Center
(
...
-- Business 2: Express Auto Service
(
...
-- Business 3: Elite Motors Workshop
(
...
-- ============================================================
-- 2. CREATE BOOKINGS WITH COMPLETE SERVICE DETAILS
-- ============================================================

-- Ensure bookings table exists with full structure
...
-- Insert comprehensive booking data (16 bookings across all garages)
...
('BK' || EXTRACT(EPOCH FROM NOW())::BIGINT || '001', '11111111-1111-1111-1111-111111111111', 'Ali Rashid Al Maktoum', '+971501111111', 'ali.rashid@email.com', 'BMW', 'X5', 2022, 'DXB-A-1234', 'Engine Diagnostics', CURRENT_DATE - 1, '09:00', 'completed', 'high', 450.00, 420.00, 'Engine sensor replacement completed'),
...
('BK' || EXTRACT(EPOCH FROM NOW())::BIGINT || '005', '22222222-2222-2222-2222-222222222222', 'Hassan Al Mansoori', '+971502222111', 'hassan.mansoori@email.com', 'Honda', 'Civic', 2019, 'DXB-E-7890', 'Transmission Check', CURRENT_DATE - 1, '08:00', 'completed', 'urgent', 800.00, 750.00, 'Transmission service completed'),
...
('BK' || EXTRACT(EPOCH FROM NOW())::BIGINT || '011', '44444444-4444-4444-4444-444444444444', 'Rashid Al Blooshi', '+971504444111', 'rashid.blooshi@email.com', 'Ford', 'F-150', 2020, 'DXB-K-1111', 'Suspension Repair', CURRENT_DATE - 1, '10:00', 'completed', 'high', 680.00, 650.00, 'Suspension components replaced'),
...
('BK' || EXTRACT(EPOCH FROM NOW())::BIGINT || '014', '55555555-5555-5555-5555-555555555555', 'Tariq Al Mulla', '+971505555111', 'tariq.mulla@email.com', 'Volkswagen', 'Tiguan', 2022, 'DXB-N-4444', 'Cooling System', CURRENT_DATE - 1, '11:00', 'completed', 'normal', 400.00, 380.00, 'Radiator service completed'),
...
-- ============================================================
-- 3. CREATE APPOINTMENTS WITH SCHEDULING DETAILS
-- ============================================================

-- Ensure appointments table exists with full structure
...
-- Insert appointment data (10 appointments across all garages)
...
-- Premium Auto Care Center Appointments (2 appointments)
...
-- Express Auto Service Appointments (2 appointments)
...
-- Elite Motors Workshop Appointments (2 appointments)
...
-- Speed Tech Garage Appointments (2 appointments)
...
-- Royal Auto Works Appointments (2 appointments)
...
-- ============================================================
-- 4. CREATE INSPECTIONS WITH COMPREHENSIVE TRACKING
-- ============================================================

-- Ensure inspections table exists with full structure
...
-- Insert comprehensive inspection data (11 inspections across all garages)
...
-- Premium Auto Care Center Inspections (3 inspections)
('INS' || EXTRACT(EPOCH FROM NOW())::BIGINT || '001', '11111111-1111-1111-1111-111111111111', 'Saif Al Rashid Nasser', '+971501111001', 'saif.rashid@email.com', 'BMW', 'X3', 2021, 'DXB-AA-1001', CURRENT_DATE - 1, '08:00', 'Ahmad Al Zaabi', 'completed', 'normal', '["Engine diagnostics", "Brake system check", "Suspension inspection", "Electrical system test"]'::jsonb, 300.00, 280.00, 'All major systems functioning well. Minor brake fluid top-up performed.', 'Regular maintenance recommended every 6 months.', 'Premium customer - VIP service'),
...
-- Express Auto Service Inspections (2 inspections)
('INS' || EXTRACT(EPOCH FROM NOW())::BIGINT || '004', '22222222-2222-2222-2222-222222222222', 'Mona Al Hashemi Khalil', '+971502222001', 'mona.hashemi@email.com', 'Honda', 'CR-V', 2019, 'DXB-DD-4004', CURRENT_DATE - 1, '09:00', 'Omar Khalil', 'completed', 'normal', '["Engine oil analysis", "Air filter inspection", "Brake pad measurement", "Tire tread check"]'::jsonb, 200.00, 180.00, 'Engine oil due for change. Air filter replaced. Brakes in good condition.', 'Next service in 3 months or 10,000km.', 'Customer enrolled in maintenance package'),
...
RAISE NOTICE '🎉 PRODUCTION DEMO DATA CREATED SUCCESSFULLY!';
RAISE NOTICE '======================================================';
RAISE NOTICE '📊 SUMMARY:';
RAISE NOTICE '   • 5 Garage Businesses with complete profiles';
RAISE NOTICE '   • 16 Bookings with full service details';
RAISE NOTICE '   • 10 Appointments with scheduling';
RAISE NOTICE '   • 11 Inspections with comprehensive tracking';
RAISE NOTICE '   • All RLS policies enabled';
RAISE NOTICE '   • Ready for employer demonstration';
RAISE NOTICE '======================================================';
...
-- Show data distribution per garage
SELECT 
    u.garage_name,
    u.email,
    ...
    COALESCE(SUM(b.estimated_cost), 0) as total_booking_revenue,
    COALESCE(SUM(i.estimated_cost), 0) as total_inspection_revenue
FROM users u
...
GROUP BY u.garage_name, u.email, u.id
...
After running this script, create authentication accounts:
...
4. Ensure User IDs match the ones in this script

📊 DATA OVERVIEW:
- Total Records: 42 (5 users + 16 bookings + 10 appointments + 11 inspections)
- Revenue Tracking: AED 12,000+ in estimated bookings and inspections
- Status Distribution: Mix of completed, in-progress, confirmed, and pending
- Vehicle Variety: 15+ different makes from BMW to Mitsubishi
- UAE Authenticity: Local names, phone numbers, and license plates
