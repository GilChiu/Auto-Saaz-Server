...
CREATE POLICY inspections_garage_owner_policy ON inspections
    USING (garage_owner_id = (SELECT id FROM users WHERE id = auth.uid()));

CREATE POLICY inspections_garage_owner_insert_policy ON inspections
    FOR INSERT WITH CHECK (garage_owner_id = (SELECT id FROM users WHERE id = auth.uid()));

CREATE POLICY inspections_garage_owner_update_policy ON inspections
    FOR UPDATE USING (garage_owner_id = (SELECT id FROM users WHERE id = auth.uid()));

CREATE POLICY inspections_garage_owner_delete_policy ON inspections
    FOR DELETE USING (garage_owner_id = (SELECT id FROM users WHERE id = auth.uid()));
...
COMMENT ON TABLE inspections IS 'Vehicle inspections and maintenance records';
COMMENT ON COLUMN inspections.tasks IS 'JSON array of inspection tasks/checklist items';
COMMENT ON COLUMN inspections.findings IS 'Inspection results and findings';
COMMENT ON COLUMN inspections.recommendations IS 'Technician recommendations for customer';