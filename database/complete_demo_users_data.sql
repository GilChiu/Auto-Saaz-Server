-- ============================================================
-- COMPLETE DEMO USERS AND DATA SETUP
-- This script creates multiple users with login credentials and their individual data
...
-- ============================================================
-- 1. CREATE DEMO USERS WITH LOGIN CREDENTIALS
-- ============================================================
...
-- You can generate these using: bcrypt.hash("Demo123!", 10)
INSERT INTO users (
    id,
    email,
    password_hash,
...
(
    ...
    '$2b$10$rQ7gE4vJ9sK2dF8nP1mL5e7XzN8wC6bH9qT3vR2aS5fG1hM4jK6lO', -- Demo123!
    'garage_owner',
...
SELECT 
    'Bookings Status Summary' as summary_type,
    status,
    COUNT(*) as count
FROM bookings b
JOIN users u ON b.garage_id = u.id
WHERE u.email LIKE '%@demo.autosaaz.com'
GROUP BY status
...
SELECT 
    ...
    COUNT(*)
FROM appointments a
JOIN users u ON a.garage_owner_id = u.id
WHERE u.email LIKE '%@demo.autosaaz.com'
GROUP BY status
...
SELECT 
    'Inspections Status Summary',
    status,
    COUNT(*)
FROM inspections i
JOIN users u ON i.garage_owner_id = u.id
WHERE u.email LIKE '%@demo.autosaaz.com'
GROUP BY status
ORDER BY summary_type, status;

RAISE NOTICE '🎉 COMPLETE DEMO SETUP FINISHED!';
...
-- ============================================================
/*
🔐 DEMO USER LOGIN CREDENTIALS (All use password: Demo123!)

1. ahmed.premium@demo.autosaaz.com - Premium Auto Care Center (Ahmed Al Mansouri)
   📊 Data: 4 bookings, 2 appointments, 3 inspections
   🚗 Specialty: Luxury vehicles (BMW, Mercedes, Audi)

2. mohammad.express@demo.autosaaz.com - Express Auto Service (Mohammad Hassan)
   📊 Data: 3 bookings, 2 appointments, 2 inspections
   🚗 Specialty: Quick service and maintenance

3. sara.elite@demo.autosaaz.com - Elite Motors Workshop (Sara Al Zahra)
   📊 Data: 3 bookings, 2 appointments, 2 inspections
   🚗 Specialty: High-end modifications (Porsche, Range Rover)

4. khalid.speedtech@demo.autosaaz.com - Speed Tech Garage (Khalid Al Rashid)
   📊 Data: 3 bookings, 2 appointments, 2 inspections
   🚗 Specialty: Performance and off-road vehicles

5. fatima.royal@demo.autosaaz.com - Royal Auto Works (Fatima Al Zaabi)
   📊 Data: 3 bookings, 2 appointments, 2 inspections
   🚗 Specialty: Family vehicles and desert preparation

TOTAL: 16 bookings, 10 appointments, 11 inspections
Each user has realistic, unique data with different service types and priorities.
*/